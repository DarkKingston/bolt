	...
	<div class="form-group">
        <label class="col-sm-3"><?=$title?> <?if($required==1){?><b class="red">*</b><?}?></label>
        	...
        	switch($type){
                ...
                ?>
                <div class="col-sm-3">
                	<input autocomplete="off" <?if($required==1){echo"required";}?> type="text"  name="<?=$name?>" <?if($action=='edit'){?>value="<?=$elementInfo[$name]?>"<?}elseif($default!=''){?>value="<?=$default?>"<?}?> class="datetimepicker <?=$sizeClass[$size]?> form-control">
                </div>
                <?    
                break;
...
function isWebMasterOffice() {
    if( define_client_ip() == '95.65.81.152' || define_client_ip() == "89.28.56.188" ){
        return true;
    }
    else {
        return false;
...
function getCatalogTree($parent = 0){
    global $db,$CCpu;
    $getSubcatalog = mysqli_query($db,"SELECT id, title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND section_id = '".$parent."' ORDER BY sort DESC ");
    if (!empty($getSubcatalog)){?>
        <ul class="index_catalog_submenu">
            <?while ($subcat = mysqli_fetch_assoc($getSubcatalog)){?>
                <a href="<?=$CCpu->writelink(71,$subcat['id'])?>" class="index_catalog_subitem">
                    <?=$subcat['title']?>
                </a>
            <?
                $checkSubcat = mysqli_query($db,"SELECT id FROM ws_catalog WHERE active = '1' AND section_id = '".$subcat['id']."' ORDER BY sort DESC ");
                $check = mysqli_fetch_assoc($checkSubcat);
                if (!empty($check)){
                    getCatalogTree2($subcat['id']);
                }
            }
            ?>
        </ul>
...
function getCatalogTree2($parent = 0){
    global $db,$CCpu;
    $getSubcatalog = mysqli_query($db,"SELECT id, section_id,title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND section_id = '".$parent."' ORDER BY sort DESC ");
    if (!empty($getSubcatalog)){?>
        <ul style="padding-left: 10px">
            <?while ($subcat = mysqli_fetch_assoc($getSubcatalog)){?>
                <li class="new_subcategory">
                    <a href="<?=$CCpu->writelink(71,$subcat['id'])?>" class="index_catalog_subitem">
                        <?=$subcat['title']?>
                    </a>
                </li>
...
function getCatalogTreeForHeader ($parent = 0){
    global $db,$CCpu;
    $getSubcatalog = mysqli_query($db,"SELECT id, title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND section_id = '".$parent."' ORDER BY sort DESC ");
    if (!empty($getSubcatalog)){?>
        <ul>
            <?while ($subcat = mysqli_fetch_assoc($getSubcatalog)){?>
                <li>
                    <a href="<?=$CCpu->writelink(71,$subcat['id'])?>" class="header_menu_item_link">
                        <?=$subcat['title']?>
                    </a>
                </li>
                <?
                ...
                $check = mysqli_fetch_assoc($checkSubcat);
                if (!empty($check)){
                    getCatalogTreeForHeader2($subcat['id']);
                }
            }
            ?>
        </ul>
...
function getCatalogTreeForHeader2($parent = 0){
    global $db,$CCpu;
    $getSubcatalog = mysqli_query($db,"SELECT id, section_id,title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND section_id = '".$parent."' ORDER BY sort DESC ");
    if (!empty($getSubcatalog)){?>
        <ul style="padding-left: 10px">
...
function getCatalogTreeElements($parent = 0){
    global $db,$CCpu;global $arrElements;
    $arrElements = [];
    $getSubcatalog = mysqli_query($db,"SELECT id, section_id,title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND section_id = '".$parent."' ORDER BY sort DESC ");
   if (!empty($getSubcatalog)){
        ...
        while ($subcat = mysqli_fetch_assoc($getSubcatalog)){?>
                <?
                array_push($arrElements, $subcat['id']);
                $checkSubcat = mysqli_query($db,"SELECT id FROM ws_catalog WHERE active = '1' AND section_id = '".$subcat['id']."' ORDER BY sort DESC ");
                $check = mysqli_fetch_assoc($checkSubcat);
                if (!empty($check)){
                    getCatalogTreeElements($subcat['id']);
                }
            }
        ?>
    <?}
}
function getCatalogTreeElements2($parent){
    global $db,$CCpu;global $arrElements;
    $arrElements = [];
    $getSubcatalog = mysqli_query($db,"SELECT id, section_id,title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND id = '".$parent."' ORDER BY sort DESC ");
    if (!empty($getSubcatalog)){
...
function getCatalogScrumb($parent = 0){
    global $db,$CCpu;
    $getSubcatalog = mysqli_query($db,"SELECT id, section_id, title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND id = '".$parent."' ORDER BY sort DESC ");
    if (!empty($getSubcatalog)){?>
        <?while ($subcat = mysqli_fetch_assoc($getSubcatalog)){?>
            ...
            <div class="">
                <a href="<?=$CCpu->writelink(71,$subcat['id'])?>" class="scrumb_item"><?=$subcat['title']?></a>
                <span class="scrumb_delimiter">/</span>
            </div>
            <?
            $checkSubcat = mysqli_query($db,"SELECT id FROM ws_catalog WHERE active = '1' AND id = '".$subcat['section_id']."' ORDER BY sort DESC ");
            $check = mysqli_fetch_assoc($checkSubcat);
            if (!empty($check)){
                getCatalogScrumb2($subcat['section_id']);
            }
        }
        ?>
    <?}
}
function getCatalogScrumb2($parent = 0){
    global $db,$CCpu;
    $getSubcatalog = mysqli_query($db,"SELECT id, section_id, title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND id = '".$parent."' ORDER BY sort DESC ");
    if (!empty($getSubcatalog)){?>
        <?while ($subcat = mysqli_fetch_assoc($getSubcatalog)){?>
...
function getCatalogScrumbProduct($parent = 0){
    global $db,$CCpu;
    $getProduct = mysqli_query($db,"SELECT id, section_id, title_".$CCpu->lang." AS title FROM ws_catalog_elements WHERE active = '1' AND id = '".$parent."' ORDER BY sort DESC ");
    if($product = mysqli_fetch_assoc($getProduct)){
        $getSubcatalog = mysqli_query($db,"SELECT id, section_id, title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND id = '".$product['section_id']."' ORDER BY sort DESC ");
    }
   if (!empty($getSubcatalog)){?>
        <?while ($subcat = mysqli_fetch_assoc($getSubcatalog)){ ?>
            ...
            <?
            $checkSubcat = mysqli_query($db,"SELECT id FROM ws_catalog_elements WHERE active = '1' AND id = '".$subcat['section_id']."' ORDER BY sort DESC ");
            $check = mysqli_fetch_assoc($checkSubcat);
            if (!empty($check)){
                getCatalogScrumbProduct2($subcat['section_id']);
            }
        }
        ?>
    <?}
}
function getCatalogScrumbProduct2($parent = 0){
    global $db,$CCpu;
    $getSubcatalog = mysqli_query($db,"SELECT id, section_id, title_".$CCpu->lang." AS title FROM ws_catalog WHERE active = '1' AND id = '".$parent."' ORDER BY sort DESC ");
    if (!empty($getSubcatalog)){?>
        <?while ($subcat = mysqli_fetch_assoc($getSubcatalog)){?>
...
function control_favorite($client,$id){

    global $db;

    $check = mysqli_query($db,"SELECT id FROM ws_favorite WHERE client_id = '".$client."' AND prod_id = '".$id."' ");
...
function control_compare($client,$id){

    global $db;

    $check = mysqli_query($db,"SELECT id FROM ws_compare WHERE client_id = '".$client."' AND prod_id = '".$id."' ");
